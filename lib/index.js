...
function _findGeometries(pg) {
    return (rows, callback) => {
        ...
        try {
            sqlStatement = sqlUtils.find(uuids);
        } catch (err) {
            return callback(err);
        }
...
module.exports = class GeoGraph {
        ...
        async.map(jsons, (item, callback) => {
            ...
            async.parallel(tasks, (err) => {
                if (err) {
                    ...
                    _handleTransactions(transactions, start, 'commit', callback);
                }
            });
        }, (err, results) => {
            if (jsons.length > 1) {
    ...
    deleteNodesById(label, uuids, callback) {
        ...
        async.parallel(tasks, (err) => {
            if (err) {
                ...
                _handleTransactions(transactions, null, 'commit', callback);
            }
        });
    }
    ...
    deleteRelationships(relations, callback) {
        let { task } = _modifyDatabase(this.db, 'deleteRelationships', [relations], callback);
