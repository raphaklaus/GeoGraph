...
function _jsonToMatchStatements(json) {
    ...
    if (!label) {
        throw new GeoGraphValidationError(`You must provide a valid label - ${json._label}`);
    }

    variables.nodes.push(id);

    statements.push({
        cypher: `MATCH (${id}:${label} {uuid: "${json.uuid}"}) WITH ${id}`
    });
...
function deleteNodesById(label, uuids) {
    ...
    return {
        cypher: `MATCH (n:${validLabel}) where n.uuid in $uuids DETACH DELETE n RETURN n`,
        params: {
            uuids: uuids
        }
    };
