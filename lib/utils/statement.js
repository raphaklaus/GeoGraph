...
function _matchRelationship(id, relationshipString) {
    ...
    _.each(relationships, (relationship) => {
        ...
        if (isOptional) {
            ...
            statement.cypher += `OPTIONAL MATCH (${id})`;
        } else {
            statement.cypher += `MATCH (${id})`;
        }

        let where = regexes.where.getGroup(relationship, 'where');
