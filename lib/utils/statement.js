...
function _matchRelationship(id, relationString) {
    let relations = relationString.split('.');

    _.each(relations, (relation) => {
        let
            relatedId = utils.getUniqueIdentifier(),
            relationId = utils.getUniqueIdentifier(),
            relationType = relation.split('-').join('|'),
        ...
        if (where) {
            let filters = _.map(regexes.propertyFilter.enumerateGroups(where, 'property', (filter)=> `${id}.${filter}`));

            if (filters.length) {
                statement.cypher += ` WHERE ${filters.join(')}`;
            }
        }
