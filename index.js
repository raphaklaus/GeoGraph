(function (exports) {
    ...
    function _createRelationships(node, trx, options, callback) {
        var
            error,
            current = 0;
    ...
    function _createGraph(node, trx, callback) {
        async.waterfall([
            ...
            function (result, callback) {
                ...
                node.uuid = result.uuid;
                _createRelationships(node, trx, null, function (err) {
    ...
    function updateGraphs(graphs, options, callback) {
                    ...
                    async.parallel([
                        (callback) => {
                            async.each(_.keys(geometries), (uuid, callback) => {
                                async.each(_.keys(geometries[uuid]), (key, callback) => {
                                    knex.raw(`INSERT INTO geometries (node_uuid, node_key, node_geometry) 
                        ...
                        (callback) => {
                            async.each(nodes, (node, callback) => {
                                _createRelationships(node, trx, options, callback);
                            }, callback);
                        }
                    ], function (err) {
                        if (err) {
    ...
    function getById(uuid, queryObject, callback) {
        async.waterfall([
            function (callback) {
                _executeStatement({
            ...
            function (nodes, callback) {
                knex('geometries')
                    ...
                    .select('node_uuid', 'node_key', 'properties',
                        knex.raw('ST_AsGeoJSON(node_geometry)::json as node_geometry'))
    ...
    function deleteNodes(uuids, callback) {
        knex.transaction((trx) => {
            async.parallel([
                (callback) => {
                    knex('geometries')
    ...
    function deleteRelationships(relationships, callback) {
        _executeStatement({
